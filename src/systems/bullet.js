/* global window.AFRAME window.ABLAST */
var PoolHelper = require('../lib/poolhelper.js')

window.ABLAST.BULLETS = {}

window.ABLAST.registerBullet = function(name, data, definition) {
	if (window.ABLAST.BULLETS[name]) {
		throw new Error(
			'The bullet `' +
				name +
...
window.AFRAME.registerSystem('bullet', {
	init: function() {
		var self = this
