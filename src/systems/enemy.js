...
window.ABLAST.ENEMIES = {}

window.ABLAST.registerEnemy = function(name, data, definition) {
	if (window.ABLAST.ENEMIES[name]) {
		throw new Error(
			'The enemy `' +
				name +
...
window.AFRAME.registerSystem('enemy', {
	schema: {
		wave: { default: 0 }
	},
	...
	init: function() {
		...
		this.poolHelper = new PoolHelper(
			'enemy',
			window.ABLAST.ENEMIES,
			this.sceneEl
		)
	...
	createWave: function(waveNumber) {
		this.currentWave = window.WAVES[waveNumber % window.WAVES.length]
