/* globals window.ABLAST window.AFRAME */
var PoolHelper = require('../lib/poolhelper.js');

window.ABLAST.EXPLOSIONS = {};

window.ABLAST.registerExplosion = function (name, data, definition) {
  if (window.ABLAST.EXPLOSIONS[name]) {
...
window.AFRAME.registerSystem('explosion', {
  schema: {
    wave: {default: 0}
  },

  init: function () {
    this.poolHelper = new PoolHelper('explosion', window.ABLAST.EXPLOSIONS, this.sceneEl);
...
/* globals window.ABLAST */
window.ABLAST.registerExplosion(
  // name
  'enemy',
  // data
  {
