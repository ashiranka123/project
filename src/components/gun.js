 ...
 */
window.AFRAME.registerComponent('shoot', {
	schema: {
	...
	init: function() {
		...
		// Add event listener.
		if (data.on) {
			el.addEventListener(data.on, this.shoot)
		}
...
*/
	...
	shoot: (function() {
		...
		var position = new window.THREE.Vector3()
		var quaternion = new window.THREE.Quaternion()
		var scale = new window.THREE.Vector3()
		var translation = new window.THREE.Vector3()
		var incVive = new window.THREE.Vector3(0.0, -0.23, -0.15)
		var incOculus = new window.THREE.Vector3(0, -0.23, -0.8)
		var inc = new window.THREE.Vector3()
		...
		return function() {
			...
			window.ABLAST.currentScore.shoots++
			...
			if (el.components['weapon']) {
				inc.copy(
					el.components.weapon.controllerModel === 'oculus-touch-controller'
						? incOculus
						: incVive
				)
			}
