...
window.AFRAME.registerSystem('highscores', {
	schema: {
	...
	init: function() {
		if (!this.isHighScore()) {
			console.warn(
				"Highscore can't be loaded or saved as no localStorage support found!"
			)
			return
		}
		...
		document
			...
			.addEventListener('click', function(event) {
				window.ABLAST.currentScore.name = document.getElementById(
					'player-name'
				).value
				self.addNewScore(window.ABLAST.currentScore)
		...
		this.sceneEl.addEventListener('gamestate-changed', function(evt) {
			if ('state' in evt.detail.diff) {
				if (
					evt.detail.state.state === 'STATE_GAME_OVER' ||
					evt.detail.state.state === 'STATE_GAME_WIN'
				) {
					ablastUI.style.display = self.isHighScore(window.ABLAST.currentScore)
						? 'block'
						: 'none'
				}
			}
	...
	shouldStoreScore: function(data) {
		return (
			this.scores.length < this.data.maxScores ||
...
window.AFRAME.registerComponent('highscores', {
	schema: {},
...
function buildText(scores) {
	...
	scores
		...
		.forEach(function appendText(score) {
			var name = score.name.toLowerCase()
