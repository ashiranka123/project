...
window.AFRAME.registerComponent('bullet', {
  schema: {
    name: { default: '' },
    direction: { type: 'vec3' },
  ...
  init: function () {
    ...
    this.bullet = window.ABLAST.BULLETS[this.data.name];
  ...
  hitObject: function (type, data) {
    ...
    if (this.data.owner === 'enemy') {
      ...
      if (type === 'bullet') {
        ...
        window.ABLAST.currentScore.validShoot++;
      }
  ...
  tick: (function () {
    ...
    return function tick (time, delta) {
      ...
      var ray = new window.THREE.Raycaster(this.temps.position, this.temps.direction.clone().normalize());
