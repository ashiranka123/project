/* globals window.AFRAME window.THREE */
window.AFRAME.registerComponent('json-model', {
	schema: {
		src: { type: 'asset' },
	...
	update: function(oldData) {
		this.loader = null
		this.helpers = new window.THREE.Group()
		this.mixers = []
		this.animationNames = []
		...
		if (this.data.singleModel) {
			...
			this.loader = new window.THREE.ObjectLoader()
	...
	onModelLoaded: function(geometry, materials) {
		...
		var mesh = new window.THREE.SkinnedMesh(geometry, materials[0])
		var self = this
		...
		if (
			mesh.geometry['animations'] !== undefined &&
			mesh.geometry.animations.length > 0
		) {
			mesh.material.skinning = true
			var mixer = { mixer: new window.THREE.AnimationMixer(mesh), clips: {} }
		...
		this.skeletonHelper = new window.THREE.SkeletonHelper(mesh)
	...
	onSceneLoaded: function(group) {
		...
		if (group['animations'] !== undefined) {
			var mixer = { mixer: new window.THREE.AnimationMixer(group), clips: {} }
	...
	addNormalHelpers: function(mesh) {
		var fnh = new window.THREE.FaceNormalsHelper(
			mesh,
			this.data.debugNormalsLength
		)
		this.helpers.add(fnh)
		var vnh = new window.THREE.VertexNormalsHelper(
			mesh,
			this.data.debugNormalsLength
		)
	...
	playAnimation: function(animationName, repeat) {
		for (var i in this.mixers) {
			...
			clip.setLoop(
				repeat ? window.THREE.LoopRepeat : window.THREE.LoopOnce,
				repetitions
			)
		}
	},
