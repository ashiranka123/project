/* globals window.AFRAME window.ABLAST window.THREE */
window.AFRAME.registerComponent('gamestate-visuals', {
	schema: {},
	init: function() {
	...
	startPlaying: function() {
		...
		var self = this
		var rotation = { x: 0.0 }
		var tween = new window.AFRAME.TWEEN.Tween(rotation)
			.to({ x: Math.PI * 0.6 }, 1000)
			.onComplete(function() {
				self.mainMenuGroup.setAttribute('visible', false)
			})
			.easing(window.AFRAME.TWEEN.Easing.Back.InOut)
			.onUpdate(function() {
	...
	finishPlaying: function(type) {
		...
		var tweenGroup = new window.AFRAME.TWEEN.Tween(groupPosition)
			.to({ y: 1 }, 1000)
			.easing(window.AFRAME.TWEEN.Easing.Elastic.Out)
			.onUpdate(function() {
		...
		var tweenReset = new window.AFRAME.TWEEN.Tween(resetPosition)
			.to({ y: 0 }, 1000)
			.delay(3000)
			.easing(window.AFRAME.TWEEN.Easing.Elastic.Out)
			.onUpdate(function() {
	...
	mainMenu: function() {
		...
		var tweenEnemy = new window.AFRAME.TWEEN.Tween(enemyPosition)
			.to({ positionY: 1.4 }, 1000)
			.delay(1000)
			.easing(window.AFRAME.TWEEN.Easing.Back.InOut)
			.onUpdate(function() {
				self.startEnemy.setAttribute('position', {
					x: 0,
					y: enemyPosition.positionY,
					z: -4
				})
			})
		tweenEnemy.start()
		...
		var tween = new window.AFRAME.TWEEN.Tween(textsPosition)
			.to({ y: -5 }, 1000)
			.easing(window.AFRAME.TWEEN.Easing.Elastic.In)
			.onComplete(function() {
		...
		var tween = new window.AFRAME.TWEEN.Tween(logoRotation)
			.to({ x: Math.PI * 0.6 }, 1000)
			.easing(window.AFRAME.TWEEN.Easing.Elastic.Out)
			.delay(1000)
			.onUpdate(function() {
