...
window.THREE.Spline = function(points) {
	this.points = points
	...
	this.getPoint = function(k) {
		...
		c[1] = intPoint
		c[2] =
			intPoint > this.points.length - 2 ? this.points.length - 1 : intPoint + 1
		c[3] =
			intPoint > this.points.length - 3 ? this.points.length - 1 : intPoint + 2
	...
	this.getLength = function(nSubDivisions) {
		var i,
			...
			intPoint = 0,
			oldIntPoint = 0,
			oldPosition = new window.THREE.Vector3(),
			tmpVec = new window.THREE.Vector3(),
			chunkLengths = [],
			totalLength = 0
 ...
 */
window.AFRAME.registerComponent('curve-movement', {
	schema: {
		debug: { default: false },
		type: { default: 'single' },
	...
	addPoints: function(points) {
		...
		this.cycleTimes = chunkLengths
			.map(function(chunkLength, i) {
				if (i === 0) {
					return null
				}
	...
	tick: function(time, delta) {
		...
		if (
			!this.isClosed() &&
			this.currentPointIndex === spline.points.length - 1
		) {
			return
		}
...
function ASpline(points) {
	var spline = new window.THREE.Spline(points)
