...
window.AFRAME.registerComponent('explosion', {
  ...
  init: function () {
    this.life = 0;
    this.starttime = null;
    this.meshes = new window.THREE.Group();
    ...
    for (var i in this.parts) {
      ...
      part.meshes = [];
      var planeGeometry = new window.THREE.PlaneGeometry(part.scale, part.scale);
      ...
      for (var n = 0; n < part.copies; n++) {
        var mesh = new window.THREE.Mesh(planeGeometry, material);
        if (!part.billboard) {
