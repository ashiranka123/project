...
window.AFRAME.registerComponent('gamestate', {
	schema: {
		health: { default: 5 },
	...
	init: function() {
		...
		registerHandler('enemy-death', function(newState) {
			newState.points++
			window.ABLAST.currentScore.points++
			...
			if (newState.numEnemies === 0) {
				...
				if (newState.numSequences === 0) {
					...
					newState.wave++
					if (newState.wave >= window.WAVES.length) {
						self.gameEnd(newState, true)
					}
				}
			}
		...
		registerHandler('player-hit', function(newState) {
			if (newState.state === 'STATE_PLAYING') {
				...
				if (newState.health <= 0) {
					...
					document.getElementById('introMusic').components.sound.playSound()
					document
						.getElementById('mainThemeMusic')
						.components.sound.pauseSound()
				}
			}
			return newState
		})
		...
		function registerHandler(event, handler) {
			el.addEventListener(event, function(param) {
				var newState = handler(window.AFRAME.utils.extend({}, state), param)
				publishState(event, newState)
			})
		}

		function publishState(event, newState) {
			var oldState = window.AFRAME.utils.extend({}, state)
			el.setAttribute('gamestate', newState)
			state = newState
			el.emit('gamestate-changed', {
				event: event,
				diff: window.AFRAME.utils.diff(oldState, newState),
				state: newState
			})
		}
	}
 ...
 */
window.AFRAME.registerComponent('gamestate-bind', {
	schema: {
		default: {},
	...
	updateBinders: function() {
		...
		function syncState(state) {
			Object.keys(state).forEach(function updateIfNecessary(stateProperty) {
				...
				if (subscribed.indexOf(stateProperty) === -1) {
					return
				}
				window.AFRAME.utils.entity.setComponentProperty(
					el,
					targetProperty,
					value
				)
