...
window.AFRAME.registerComponent('enemy', {
  schema: {
    name: {default: 'enemy0'},
  ...
  init: function () {
    this.alive = true;
    this.hipBone = null;
    this.definition = window.ABLAST.ENEMIES[this.data.name].definition;
    this.definition.init.call(this);
    var comp = window.ABLAST.ENEMIES[this.data.name].components.enemy;
    this.maxhealth = this.health = comp.health;
    this.color = comp.color;
    this.scale = comp.scale;
    this.gunOffset = new window.THREE.Vector3(0.0, 0.44, 0.5).multiplyScalar(this.scale);
    ...
    // gun glow
    this.gunGlowMaterial = new window.THREE.MeshBasicMaterial({
      color: this.color,
    ...
    this.gunGlow = new window.THREE.Mesh(new window.THREE.PlaneGeometry(this.scale, this.scale), this.gunGlowMaterial);
  ...
  collided: function () {
    ...
    if (this.health <= 0) {
      ...
      this.whiteMaterial = new window.THREE.MeshBasicMaterial({color: this.color, transparent: true });
