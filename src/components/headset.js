...
window.AFRAME.registerComponent('headset', {
	schema: {
		on: { default: 'click' }
	},
	...
	updatePose: (function() {
		var controllerEuler = new window.THREE.Euler()
		var controllerPosition = new window.THREE.Vector3()
		var controllerQuaternion = new window.THREE.Quaternion()
		var dolly = new window.THREE.Object3D()
		var standingMatrix = new window.THREE.Matrix4()
		...
		return function() {
			...
			el.setAttribute('rotation', {
				x: window.THREE.Math.radToDeg(controllerEuler.x),
				y: window.THREE.Math.radToDeg(controllerEuler.y),
				z: window.THREE.Math.radToDeg(controllerEuler.z)
			})
